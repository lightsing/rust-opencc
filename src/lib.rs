...
const RAW_DICT_TS: &'static str = include_str!("dict/ts.txt");
const RAW_DICT_ST: &'static str = include_str!("dict/st.txt");
...
#[test]
fn test_dict_ts() {
    let dict = Dict::load(RAW_DICT_TS);
...
#[test]
fn test_dict_st() {
    let dict = Dict::load(RAW_DICT_ST);
    let sc = "夸夸其谈 夸父逐日
    我干什么不干你事。
    太后的头发很干燥。
    燕燕于飞，差池其羽。之子于归，远送于野。
